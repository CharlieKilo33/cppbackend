...
[[nodiscard]] Args ParseCommandLine(int argc, const char* const argv[]) {
  ...
  desc.add_options()("help,h", "produce help message")(
      "tick-period,t", po::value(&args.tick_period)->value_name("milliseconds"s),
      "set tick period")(
      "config-file,c", po::value(&args.config_file)->value_name("file"s),
      "set config file path")("www-root,w", po::value(&args.www_root)->value_name("dir"s),
                              "set static files root")(
      "randomize-spawn-points", po::value(&args.randomize_spawn_points),
  ...
  if (!vm.contains("config-file"s)) {
    ...
    BOOST_LOG_TRIVIAL(error) << logger::CreateLogMessage(
        error_msg, logger::ExitCodeLog(EXIT_FAILURE));

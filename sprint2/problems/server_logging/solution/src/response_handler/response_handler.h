...
bool IsApiRequest(const http::request<Body, http::basic_fields<Allocator>>& req) {
  return req.target().substr(1, 3) == "api";
}

template <typename Body, typename Allocator>
bool IsGetMapListRequest(const http::request<Body, http::basic_fields<Allocator>>& req) {
  return (req.target() == "/api/v1/maps") || (req.target() == "/api/v1/maps/");
}

template <typename Body, typename Allocator>
StringResponse HandleGetMapList(const http::request<Body, http::basic_fields<Allocator>>& req, const model::Game& game) {
...
template <typename Body, typename Allocator>
bool IsGetMapByIdRequest(const http::request<Body, http::basic_fields<Allocator>>& req, const model::Game& game) {
  auto pathComponents = SplitStr(req.target());
  bool isValidPath = (pathComponents.size() == kExpectedPathSizeForMap) && (pathComponents[kApiIndex] == "api") && (pathComponents[kVersionIndex] == "v1") && (pathComponents[kMapsIndex] == "maps");
  bool mapExists = (game.FindMap(model::Map::Id(std::string(pathComponents[kMapIdIndex]))) != nullptr);
  return isValidPath && mapExists;
}

template <typename Body, typename Allocator>
StringResponse HandleGetMapById(const http::request<Body, http::basic_fields<Allocator>>& req, const model::Game& game) {
  StringResponse response(http::status::ok, req.version());
  auto id = SplitStr(req.target())[kMapIdIndex];
  response.set(http::field::content_type, "application/json");
  response.body() = jsonOperation::SerializeMapToJson(*game.FindMap(model::Map::Id(std::string(id))));
  response.content_length(response.body().size());
  response.keep_alive(req.keep_alive());
  return response;
}
...
template <typename Body, typename Allocator>
bool IsBadRequest(const http::request<Body, http::basic_fields<Allocator>>& req) {
  auto pathComponents = SplitStr(req.target());
  bool isApiPath = (!pathComponents.empty()) && (pathComponents[kApiIndex] == "api");
  bool isInvalidSize = (pathComponents.size() > kMaxPathSizeForBadRequest) || (pathComponents.size() < kMinPathSizeForBadRequest);
...
template <typename Body, typename Allocator>
StringResponse HandleBadRequest(const http::request<Body, http::basic_fields<Allocator>>& req) {
...
template <typename Body, typename Allocator>
bool IsMapNotFoundRequest(const http::request<Body, http::basic_fields<Allocator>>& req, const model::Game& game) {
  auto pathComponents = SplitStr(req.target());
  bool isValidPath = (pathComponents.size() == kExpectedPathSizeForMap) && (pathComponents[kApiIndex] == "api") && (pathComponents[kVersionIndex] == "v1") && (pathComponents[kMapsIndex] == "maps");
...
template <typename Body, typename Allocator>
StringResponse HandleMapNotFound(const http::request<Body, http::basic_fields<Allocator>>& req) {
...
template <typename Body, typename Allocator>
StringResponse HandlePageNotFound(const http::request<Body, http::basic_fields<Allocator>>& req) {
  StringResponse response(http::status::not_found, req.version());
  response.set(http::field::content_type, "application/json");

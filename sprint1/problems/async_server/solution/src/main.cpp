...
StringResponse HandleRequest(http::request<http::string_body>&& req) {
    ...
    if (req.method() == http::verb::get) {
        ...
        std::string name = target.erase(0, 1);
        return text_response(http::status::ok, "Hello, " + name);
        ...
        StringResponse response = MakeStringResponse(http::status::method_not_allowed, body, req.version(), req.keep_alive());
